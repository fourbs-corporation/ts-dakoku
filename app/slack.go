...
func (ctx *Context) getActionCallback(data *slack.AttachmentActionCallback) (*slack.Msg, string, error) {
	...
	case actionTypeOnTime:
		{
			startTime := time.Date(year, month, 4, 9, 0, 0, 0, time.UTC) // 定時出勤時刻
			endTime := time.Date(year, month, 4, 18, 0, 0, 0, time.UTC) // 定時退勤時刻
			restStartTime := time.Date(year, month, 4, 12, 0, 0, 0, time.UTC) // 定時休憩開始時刻
			restEndTime := time.Date(year, month, 4, 13, 0, 0, 0, time.UTC) // 定時休憩終了時刻
