	...
	case actionTypeLeave:
		{
			...
			text = "【" + selectedTimeStr + "】" + "退勤しました :house::running:"
		}
	case actionTypeRest:
		{
	...
	case actionTypeAttend:
		{
			...
			text = "【" + selectedTimeStr + "】" + "出勤しました :office::running:"
		}
	}
	...
	if text == "today" {
		...
		for i, item := range items {
			if item.IsAttendance() && item.From.Valid {
				slackMsg += "出勤時間: " + strconv.Itoa(item.From.Int64) + "\n"
			} else if item.IsAttendance() && item.To.Valid {
				slackMsg += "出勤時間: " + strconv.Itoa(item.To.Int64) + "\n"
			}
		}
		return &slack.Msg{
			Text: slackMsg,
